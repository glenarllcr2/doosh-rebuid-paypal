...
@push('scripts')
    <script>
        ...
        // تابع برای حذف عکس از گالری و ارسال درخواست به سرور
        function deletePhoto(index) {
            console.log(index);
            console.log(userData.gallery);
            photo_id = userData.gallery[index].id;
            console.log(photo_id);
            const deleteUrl =
                `{{ route('users-manager.user.delete-photo', ['user' => $user->id, 'photoId' => '__PHOTO_ID__']) }}`
                .replace('__PHOTO_ID__', userData.gallery[index].id);
            ...
            fetch(deleteUrl, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        userData.gallery.splice(index, 1); // حذف عکس از آرایه گالری
                        renderGallery(); // رندر مجدد گالری
                        showToast("Photo deleted successfully.", "success");
                    } else {
                        showToast(data.message || "Failed to delete photo.", "danger");
                    }
                })
                .catch(error => {
                    console.error("Error deleting photo:", error);
                    showToast("An error occurred while deleting the photo.", "danger");
                });
        }
